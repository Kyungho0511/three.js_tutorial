...
while (graves.children.length < GRAVE_COUNT) {
    ...
    if (grave.position.x < WALL_WIDTH / 2 && grave.position.x > -WALL_WIDTH / 2) continue;
    grave.position.z = Math.random() * FLOOR_DEPTH - FLOOR_DEPTH / 2;
    if (grave.position.z < WALL_DEPTH / 2 && grave.position.z > -WALL_DEPTH / 2) continue;
